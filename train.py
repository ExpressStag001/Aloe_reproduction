...
def main(args):
    ...
    save_path = Path(args.save_dir) / cfg_name / f"{args.seed}_ckpt"
    ...
    if ckpt_file:
        optimiz.load_state_dict(ckpt["optimiz"])
        optimiz.gscale.load_state_dict(ckpt["gscale"])
    ...
    for cb in cfg.callback_t + cfg.callback_v:
        ...
        elif cb.type.__name__ == "SaveCkpt":
            cb.save_dir = f"{save_path}"
    ...
    if ckpt_file:
        pack.epoch = ckpt["epoch"] + 1
        pack.epoch_v = ckpt["epoch_v"]
        pack.step_count = ckpt["step_count"]
        print(f"模型加载成功, 即将执行 epoch {pack.epoch}")
    ...
    while pack.step_count < pack.total_step:
        ...
        flag1 = pack.step_count >= (pack.epoch_v + 1) * pack.val_interval
...
def parse_args():
    ...
    parser.add_argument(
        "--cfg_file",
        type=str,
        default="config-savi_aloe/savi_aloe-clevrer_video-slotformer.py",
    )
    parser.add_argument(
        "--data_dir", type=str, default="../datasets"
    )
    parser.add_argument("--save_dir", type=str, default="archive-savi_aloe")
    parser.add_argument(
        "--ckpt_file",
        type=str,
        nargs="+",
