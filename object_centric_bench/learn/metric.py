...
class MetricWrap(nn.Module):
    ...
    def _forward(self, **pack: dict) -> dict:
        ...
        for key, value in self.metrics.items():
            ...
            # +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            if any(v is None for v in kwds.values()):
                metrics[key] = (
                    pt.zeros(1).cuda(),  # loss = 0
                    pt.zeros(1, dtype=pt.bool).cuda()  # valid = False
                )
                continue
            # +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
class Metric(ABC, nn.Module):
    ...
    def finaliz(self, metric, valid=None) -> tuple:
        ...
        if self.mean is None:
            ...
            if metric.ndim > 1:  # tensor.ndim 表示 tensor 一共有多少维
                metric2 = metric.flatten(1).mean(1)  # (b,c1,c2...) -> (b,c1*c2*...) -> (b,) , flatten(1) 表示保留第 0 维（batch），把后面所有维度拉平成一维，再取平均
            else:
                metric2 = metric
        else:
...
class BinaryCrossEntropyLoss(Metric):
    ...
    def forward(self, input, target):
        ...
        """
        # loss = ptnf.cross_entropy(input, target, reduction="none")  # (b,..)
        loss = ptnf.binary_cross_entropy_with_logits(input, target)[None]  # (b=1,)
        return self.finaliz(loss)  # (b,) (b,)
